...
public class UserAuthService extends DefaultOAuth2UserService implements UserDetailsService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        List<GrantedAuthority> authorityList = getUserAuthorityList(oAuth2User);
        saveUser(attributes);
        return new DefaultOAuth2User(authorityList, attributes, GITHUB_NAME_ATTRIBUTE_KEY);
    }

    private List<GrantedAuthority> getUserAuthorityList(OAuth2User oAuth2User){
        List<GrantedAuthority> authorityList = AuthorityUtils.createAuthorityList();
        Optional<User> user = userRepository.findByName(oAuth2User.getAttributes().get(GITHUB_NAME_ATTRIBUTE_KEY).toString());
        if (user.isPresent()) {
            authorityList.addAll(oAuth2User.getAuthorities());
            authorityList.add(new SimpleGrantedAuthority(getUserRole(user.get())));
        }
